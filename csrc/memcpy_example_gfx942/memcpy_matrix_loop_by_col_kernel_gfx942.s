.text
.global memcpy_matrix_loop_by_col_kernel_gfx942
.p2align 8
.type memcpy_matrix_loop_by_col_kernel_gfx942,@function
...
.set v_tmp,             32
    ...
    s_lshl_b32      s[s_tmp+1], s[s_bx],    2           ; blockIdx * float
    s_waitcnt       lgkmcnt(0)
    s_mul_i32       s[s_tmp+1], s[s_tmp+1],     s[s_tilek]  ; blockIdx * tile_k * float
    s_lshr_b32      s[s_tilek], s[s_tilek],     2           ; tile_k / 4
    s_sub_u32       s[s_tmp],   s[s_tilek],     1
    v_and_b32       v[v_tmp],   v0,             s[s_tmp]    ; threadIdx % (tile_k/4)
    v_lshrrev_b32   v[v_tmp+1], s[s_log_tilek], v0          ; threadIdx / tile_k
    v_lshlrev_b32   v[v_tmp+1], 2,              v[v_tmp+1]  ; threadIdx / tile_k * float

    v_lshl_add_u32      v[v_tmp],   v[v_tmp],   4,  s[s_tmp+1]  ; blockIdx * tile_k * float + threadIdx % (tile_k/4) * float4
    v_mul_u32_u24       v[v_tmp+1], v[v_tmp+1], s[s_k]          ; threadIdx / tile_k * K * float
    v_add_u32           v[v_offset+0],   v[v_tmp],   v[v_tmp+1]

    s_lshl_b32 s[s_tmp],  s[s_blk_stride],  2            ; stridem * K * float
    ...
    s_lshl_b32 s[s_stride_block],   s[s_tmp],   2   ; unroll 4, 4 * stridem * K * float
...
.rodata
.p2align 6
.amdhsa_kernel memcpy_matrix_loop_by_col_kernel_gfx942
...
amdhsa.version: [ 1, 0 ]
amdhsa.kernels:
  - .name: memcpy_matrix_loop_by_col_kernel_gfx942
    .symbol: memcpy_matrix_loop_by_col_kernel_gfx942.kd
    .sgpr_count: 32
    .vgpr_count: 64
